...
You may obtain evidence only by calling the `cass_evidence` tool. That tool routes through a trusted policy broker. You may choose only:
- an allowed `intent`
...
`live.evidence` reads one policy-approved file or listing from a Cassandra endpoint agent, and it is the only channel whose result arrives in `data` rather than `items`. `data` is the agent's own structure -- a listing has `entries`, a read has `content`, a stat has `mode` and `size` -- because flattening a file read into rows would hide the thing the operator asked to see.
